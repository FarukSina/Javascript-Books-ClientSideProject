...
const Books = () => {
  ...
  const searchField = searchText =>{
    setFilteredBooks(books.filter(book => book.name.toLowerCase().includes(searchText) || book.author.toLowerCase().includes(searchText) ));
  }
  const deleteBook = (id) => {
    ...
    books ? (
      <>
        ...
        <div className={classes.searchField}>
          <label>Search : </label>
          <input type="text" name="searchText" placeholder="Search Author or Name" onChange={onChange} />
        </div>
          <Table className="mt-3" striped bordered hover variant="dark">
            ...
            <tbody>
              {(filteredBooks || books).map(({author,name,genre,price,date, _id}, i) => (
                <tr key={i}>
                  <td >{author}</td>
                  <td >{name}</td>
                  <td >{genre}</td>
